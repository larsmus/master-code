...
parser.add_argument("--n_instances", type=int, default=1024, help="Size of dataset that the model is trained on")
parser.add_argument("--dataset", type=str, default="polygons", help="which dataset to run")
parser.add_argument("--input_dim", type=int, default=28*28, help="dimension of input space")
...
def analyze_prior():
    ...
    if os.path.exists(f"./in_out/storage/{opt.dataset}/test_{opt.n_instances}_{opt.experiment}.npy"):
        test_data = np.load(f"./in_out/storage/{opt.dataset}/test_{opt.n_instances}_{opt.experiment}.npy")
        test_labels = np.load(f"./in_out/storage/{opt.dataset}/test_{opt.n_instances}_{opt.experiment}_labels.npy")
...
def walk_latent_space(row=8, interval=3):
    ...
    save_image(samples.view(row * row, 1, img_size, img_size),
               f"./in_out/storage/results/{opt.experiment}_{opt.n_instances}/interpolations_{interval}.png", nrow=row)
...
def explore_latent_variables(row=8, interval=3):
    ...
    torch.manual_seed(0)
    vae = model_data["model"]
    data = np.load(f"./in_out/storage/{opt.dataset}/test_{opt.n_instances}_{opt.experiment}.npy")
    ...
    save_image(samples.view(latent_dim * row, 1, img_size, img_size),
               f"./in_out/storage/results/{opt.experiment}_{opt.n_instances}/latent_walk_{interval}.png", nrow=latent_dim)
...
if __name__ == "__main__":
    ...
    model_data_2 = torch.load("./in_out/storage/models/vae_dsprites_latent_2.pt", map_location=torch.device('cpu'))
    model_data_8 = torch.load("./in_out/storage/models/vae_dsprites_latent_8.pt", map_location=torch.device('cpu'))
    model_data_32 = torch.load("./in_out/storage/models/vae_dsprites_latent_32.pt", map_location=torch.device('cpu'))
    model_data_128 = torch.load("./in_out/storage/models/vae_dsprites_latent_128.pt", map_location=torch.device('cpu'))

    loss = [model_data_2["train_loss"], model_data_8["train_loss"], model_data_32["train_loss"],
